...
//---------------------------------------------------------------------------
digitalWriteRGB(uint8_t red, uint8_t green, uint8_t blue)
{
#ifdef ARDUBOY_10
...
//	battery.begin();
  ...
  // flashlight mode for arduboy, hold up while booting to flash new game
  if(buttons.pressed(BTN_UP))
  {
    bitClear(PORTD, PORTD4);
    SPDR = 0xA5;
    asm volatile("nop");
    while (!(SPSR & _BV(SPIF))) { }
    bitSet(PORTD, PORTD4);
	...
	if(startMenuTimer)
	{
		...
		while(1)
		{
			if(update())
			{
				...
				// draw graphics
				display.drawBitmap(32, 0, gamebuinoLogo);
				if(logo)
				{
					display.drawBitmap(16, 16+logoOffset, logo);
				}
				display.cursorX = 16;
				display.cursorY = 16;
				display.print(name);
